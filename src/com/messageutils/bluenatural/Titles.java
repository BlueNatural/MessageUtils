...
public class Titles {
	public static MessageUtils plugin;
	...
	public static void sendTitle(Player player, String title, String subtitle, Integer fadeIn, Integer stay,
		...
		MessageUtils.getPlugin().getServer().getPluginManager().callEvent(new SendingTitleEvent(player,title,subtitle));
	}
	...
	public static void sendSubtitle(Player p,String subtitle,int fadein,int stay,int fadeout) {
		...
		sendTitle(p,"",subtitle,fadein,stay,fadeout);
		MessageUtils.getPlugin().getServer().getPluginManager().callEvent(new SendingTitleEvent(p,null,subtitle));
		
		
	}
	public static void cleartitle(Player p) {
		sendTitle(p,"","",null,null,null);
	}
	public static void animationTitle(Player p,String title,String subtitle,int fadein,int stay,int fadeout,int count,int delay) throws InterruptedException {
		
		for(int i = 0;i < count;i++) {
			
		   Thread.sleep(delay * System.currentTimeMillis());
		   sendTitle(p,title,subtitle,fadein,stay,fadeout);
		   MessageUtils.getPlugin().getServer().getPluginManager().callEvent(new SendingTitleEvent(p,title,subtitle));
		}
